...
const gameSlice = createSlice({
    ...
    reducers: {
        updateGameState(state, { payload }) {
            console.log(`${state.gameState} to ${payload}`)
        ...
        createGame(state, { payload }) {
            ...
            state.colors = colors
            // 初始化棋盘
            state.board = Array.from({ length: rows }).map((_, rIndex) =>
        ...
        finishMovingCode(state, { payload }) {
            const { position, color, gameState } = payload
            _updateCell(state, {
                position,
                state: 'set',
                color,
