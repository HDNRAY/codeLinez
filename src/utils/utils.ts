...
// 寻路
const positionToIndex = (p: Position) => `${p.x}${p.y}`;
...
const checkLine = (cell: Cell, rows: number, columns: number, getNext: (position: Position) => Cell) => {
    ...
    const line = [position];
    while (next && next.position.x < columns && next.position.y < rows && next.state === 'set' && next.color === color) {
...
export { generatePositions, generateColors, comparePositions, findPath, checkLines };